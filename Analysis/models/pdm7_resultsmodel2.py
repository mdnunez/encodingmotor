# pdm7_resultsmodel2.py - Plots results of preregistered JAGS models
#
...
# ====         ================                       ======================
# 08/22/17      Michael Nunez                             Original code
...
modelfile = loadloc + '/' + 'behavmodel_all_minBETA_randomAug_21_17_19_57.mat'
...
plt.figure()
jellyfish(samples['betacond'])
plt.title('Condition-level Beta latencies', fontsize=fontsize)

# Calculate Bayes Factors
possamps = samples['betagammacond']
# Number of chains
nchains = possamps.shape[-1]
# Number of samples per chain
nsamps = possamps.shape[-2]
...
jellyfish(samples['betagammacond'][0])

plt.plot([1, 1], [0, 7], 'r--', linewidth=2)
plt.plot([0, 0], [0, 7], 'b--', linewidth=2)
plt.xlim((-2,3))
# plt.title('Condition-level beta effects', fontsize=fontsize)
...
plt.xlabel(
    'Effect of beta latency on non-decision time (ms increase per ms increase in beta latency)', fontsize=fontsize)
...
plt.savefig(saveloc + '/nondecisiontime_effects_beta.png', dpi=300)

plt.figure()
jellyfish(samples['betagammacond'][1])
plt.title('Effect of beta latency on drift-rate', fontsize=fontsize)
plt.figure()
jellyfish(samples['betagammacond'][2])
plt.title('Effect of beta latency on boundary separation',
          fontsize=fontsize)


axes2 = plt.figure().add_subplot(111)
ultdata = np.reshape(samples['betagammault'][0,0,:],(nchains * nsamps))
...
for b in range(0, 2):
    ...
    if b == 1:
        ...
        plt.setp(mmean, markersize=10, color=teal)
plt.plot([1, 1], [0, 12], 'r--', linewidth=2)
plt.plot([0, 0], [0, 12], 'b--', linewidth=2)
plt.xlim((-4,5))
# Prior density of effect parameters
denom = stats.norm.pdf(1, loc=1, scale=3)
bf[6] = kde(1) / denom
plt.annotate('log(BF): %3.2f' % np.log(bf[6]), (2.5, 6.5), fontsize=fontsize)
...
figManager = plt.get_current_fig_manager() #Maximize screen
figManager.window.showMaximized()
plt.savefig(saveloc + '/nondecisiontime_expeffects_beta.png', dpi=300)
